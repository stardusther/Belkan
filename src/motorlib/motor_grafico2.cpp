...
void botonElegirMapaCB(int valor) {
  ...
  panelSelecMapa->add_radiobutton_to_group(group, "Nivel 3: Optimo en coste (tres objetivos)");
  panelSelecMapa->add_radiobutton_to_group(group, "Nivel 4: Reto                            ");
...
void lanzar_motor_grafico(int argc, char **argv) {
    ...
    //Main Window
    ventanaPrincipal = glutCreateWindow("Practica 2: Curso 20/21");
...
void lanzar_motor_grafico_verOnline(int argc, char **argv, EnLinea &argumentos) {
    ...
    //Main Window
    ventanaPrincipal = glutCreateWindow("Practica 2. Version BAJO PARAMETROS");
    //Main Window callback function
    glutReshapeFunc(reshape);
    glutDisplayFunc(display_ventPrincipal);
